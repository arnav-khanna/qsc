...
QSC3 is an experimental lossless compression project written in C. It packages one or more input files into a `.qsc` archive using optional reversible text transforms, a custom LZ-style tokenizer, adaptive probability models, adaptive match-length coding, and an in-tree binary arithmetic coder.
...
QSC3 splits files into 8 MiB chunks. Raw chunks may use up to 512 KiB of carry-over history from the previous chunk as match context. The carry-over bytes are available to the match finder, but are not emitted again in the decompressed output. Text-transformed chunks are encoded independently so the transform can remain self-contained.

Within each chunk, the compressor first tests the original bytes and, for textual chunks, one or more reversible text-token transforms. It keeps the representation that produces the smallest final encoded chunk.
...
- Redundancy reduction happens in the optional text transforms and in the LZ tokenizer, which replaces repeated byte ranges with length/offset instructions.
...
The remaining payload is an arithmetic-coded stream with this logical order:
...
- The new text transforms improve some local text/JSON cases but are not a substitute for a production static dictionary such as Brotli's.

## Future Improvements

- Add unit tests for range coding, text transforms, LZ tokenization, adaptive length coding, archive roundtrips, and malformed input handling.
...
- Do optional text transforms improve compression enough to justify their speed and format complexity?
